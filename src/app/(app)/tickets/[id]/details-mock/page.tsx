...
export default function TicketDetailMockPage() {
	...
	type Role = 'SUPPLIER' | 'CUSTOMER' | 'TRANSPORTER' | 'ADMIN';
	const [currentRole, setCurrentRole] = useState<Role>('SUPPLIER');
	useEffect(() => {
		// permite testar com ?role=SUPPLIER|CUSTOMER|TRANSPORTER|ADMIN
		try {
	...
	const isSupplier = currentRole === 'SUPPLIER' || isAdmin;
	const canManage = isSupplier; // editar, status, compartilhar, arquivar
	const canUpload = isSupplier; // anexar arquivos
	const canShare = isSupplier; // gerar/copiar/revogar link
	const canChangeStatus = isSupplier; // mudar status
	const canExportXML = isSupplier || isAdmin; // PDF é livre; XML opcionalmente restrito
	...
	async function handleUploadMock(file: File) {
		if (!canUpload) {
			toast.warning('Permissão negada', {
				description: 'Você não pode anexar arquivos neste ticket.',
			});
			return;
		}
		if (!file) return;
				...
				events: [
					{
						...
						meta: { name: file.name, by: currentRole },
						createdAt: new Date().toISOString(),
					},
					...prev.ticket.events,
				],
		...
		toast.success('Upload concluído', { description: `${file.name} anexado` });
	}

	function handleChangeStatus(to: string) {
		if (!t) return;
		if (!canChangeStatus) {
			toast.warning('Permissão negada', { description: 'Você não pode alterar o status.' });
			return;
		}
		...
		setData((prev: any) => ({
			ticket: {
				...
				shareToken:
					to === 'SHARED'
						? (prev.ticket.shareToken ?? crypto.randomUUID().slice(0, 16))
						: to !== 'SHARED'
							? null
							: prev.ticket.shareToken,
				events: [
					{
						id: crypto.randomUUID(),
	...
	function handleShareCopy() {
		...
		if (!canShare) {
			toast.warning('Permissão negada', {
				description: 'Você não pode copiar o link de compartilhamento.',
			});
			return;
		}
	...
	function handleRevokeShare() {
		if (!t) return;
		if (!canShare) {
			toast.warning('Permissão negada', { description: 'Você não pode revogar o link.' });
			return;
		}
		setData((prev: any) => ({ ticket: { ...prev.ticket, shareToken: null } }));
		toast('Link revogado');
	}
	...
	function handleExportXML() {
		if (!t) return;
		if (!canExportXML) {
			toast.warning('Permissão negada', { description: 'Você não pode exportar XML.' });
			return;
		}
					...
					<div className="flex items-center gap-2">
						...
						{canUpload ? (
							...
							<Tooltip>
								...
								<TooltipContent>Disponível apenas para o emissor</TooltipContent>
							</Tooltip>
						)}
						...
						{canChangeStatus ? (
							...
							<Tooltip>
								...
								<TooltipContent>Apenas o emissor pode mudar status</TooltipContent>
							</Tooltip>
						)}
						...
						<DropdownMenu>
							...
							<DropdownMenuContent align="end">
								...
								{!canManage ? null : (
									<>
										<DropdownMenuSeparator />
										<DropdownMenuLabel>Ações restritas</DropdownMenuLabel>
							...
							<CardContent>
								...
								<div className="mt-4">
									...
									{canUpload ? (
										...
										<div className="text-muted-foreground mt-1 text-xs">Apenas o emissor pode anexar arquivos.</div>
									)}
								</div>
							</CardContent>
						</Card>
											...
											<div className="flex items-center gap-2">
												...
												{canShare ? (
													...
													<Tooltip>
														...
														<TooltipContent>Apenas o emissor pode copiar/revogar</TooltipContent>
													</Tooltip>
												)}
											</div>
										)}
