...
export async function GET(request: NextRequest) {
	...
	try {
		let orderByQuery: Prisma.TransporterOrderByWithAggregationInput | undefined = { name: 'desc' };
		...
		const query: Prisma.TransporterFindManyArgs = {
			where: {
				name: search
					? {
							contains: search,
		...
		const [transporters, count] = await prisma.$transaction([
		...
		console.error('Listing transporters route error: ', error);
